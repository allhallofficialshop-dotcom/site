...
function productCard(p){
  ...
  const badge = hasDiscount ? `<span class='badge absolute left-2 top-2'>-disc</span>`.replace('disc', (typeof p.discount==='object' && p.discount.type==='fixed')? (p.discount.value+' LKR') : ((typeof p.discount==='object'?p.discount.value:p.discount)+'%')) : '';
  const priceHtml = hasDiscount ? `<div><span class="opacity-70 line-through mr-2">${fmt(p.price||0)}</span><span class="text-yellow-300">${fmt(finalPrice)}</span></div>` : `<div class="text-yellow-300">${fmt(finalPrice)}</div>`;
