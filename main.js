...
function productCard(p){
  ...
  const badge = hasDiscount
    ? `<span class='badge absolute left-2 top-2'>-disc</span>`
        .replace('disc',
          (typeof p.discount==='object' && p.discount.type==='fixed')
            ? (p.discount.value+' LKR')
            : ((typeof p.discount==='object'?p.discount.value:p.discount)+'%'))
    : '';
...
function makeOrderId(){ const d = new Date(); const ymd = d.toISOString().slice(0,10).replace(/-/g,''); const rand = Math.random().toString(36).slice(2,8).toUpperCase(); return `AHS-${ymd}-${rand}`; }
async function uploadToCloudinary(file, preset){ const url = `https://api.cloudinary.com/v1_1/${CLOUDINARY_CLOUD}/upload`; const fd = new FormData(); fd.append('file', file); fd.append('upload_preset', preset); const r = await fetch(url, { method:'POST', body: fd }); if(!r.ok) throw new Error('Upload failed'); return r.json(); }
