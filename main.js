...
function getCart(){
  ...
  document.querySelectorAll('#navCartCount')
    .forEach(el=> el.textContent = c.length ? '('+c.length+')' : '');
  return c;
}
...
function productCard(p){
  ...
  const priceHtml = hasDiscount
    ? `<div>
         <span class="opacity-70 line-through mr-2">${fmt(p.price||0)}</span>
         <span class="text-yellow-300">${fmt(finalPrice)}</span>
       </div>`
    : `<div class="text-yellow-300">${fmt(finalPrice)}</div>`;
  return `<div class="glass card rounded-2xl overflow-hidden">
    <a href="product.html?id=${p.id}">
      <img src="${p.images?.[0]||'assets/product-placeholder.png'}"
    ...
    <div class="p-3 space-y-1 relative">
      ...
      <button class="btn-glow px-3 py-1 rounded-xl"
        onclick='addToCart({id:"${p.id}", name:"${p.name}", price:${finalPrice}, image:"${p.images?.[0]||''}", qty:1, variant:null})'>
        Add
      </button>
    </div>
  </div>`;
